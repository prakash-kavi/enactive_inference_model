...
L3 maintains the learned policy prior (paper: ell_pi(s)) as dependently originated
latent tendencies, updated by experience, and writes it to the L2<->L3 blanket.
...
class Layer3Monitor(nn.Module):
    ...
    def __init__(self, blanket_l2l3: Optional[MarkovBlanketL2L3] = None):
        ...
        self._learned_log_prior: dict = {}

    def reset(self) -> None:
        """Reset monitor state for an isolated training run."""
        self.meta_awareness_ema = None
        self._learned_log_prior.clear()
    ...
    def _get_prior_for_state(self, current_state: str) -> np.ndarray:
        ...
        if current_state not in self._learned_log_prior:
            return np.zeros(4, dtype=np.float64)
        return np.array(self._learned_log_prior[current_state], dtype=np.float64)
    ...
    def update_policy_state(self, current_state: str, q_pi: np.ndarray) -> None:
        """Update learned policy prior from experience (EMA toward observed policy posterior).
        Uses L3_POLICY_LR from first update; prior is initialized to zeros when first seen.
        """
        ...
        q = q / q.sum()
        target_log = np.log(q)
        prev = self._learned_log_prior.get(
            current_state, np.zeros(4, dtype=np.float64)
        )
        self._learned_log_prior[current_state] = (
            (1.0 - L3_POLICY_LR) * prev + L3_POLICY_LR * target_log
        )
